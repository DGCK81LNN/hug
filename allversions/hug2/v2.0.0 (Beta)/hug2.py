import turtle # Turtle绘图用
import random # 备用
import codecs # 读取文件字符集问题用
import time # 时间延迟用
import json # 解码数据文件用
import re # 匹配要说的话用
import os # 更改控制台标题用

VERSION='2.1.0 Beta' # 程序版本
GENDERS={ # 男女有别
1: '男',
2: '女'
}
COLORS={ # 颜色统一规定表
1: '#48F', # 男生
2: '#D18', # 女生
'green': '#080'
}

load0='-----loading...-----' # 加载进度条（未点亮）
load1='>>>>>LOADING~~~>>>>>' # 加载进度条（点亮）
showdebug=False # 是否显示debug信息，输入DEBUG切换
t=None # 存放Turtle

def pl(text, end="\n", wait=0.5, delay=0.1):
    """逐字输出字符串

    参数：
    text(0): 要输出的文本
    end: 换行符
    wait: 输出完成后等待的时间
    delay: 每个字符等待的时间
    """
    for i in range(len(text)):
        print(text[i], end='', flush=True)
        time.sleep(delay)
    time.sleep(wait)
    print(end,end='')

def il(prompt):
    """逐字输出提示文本，然后要求用户输入

    参数：
    prompt(0): 提示文本
    返回值：用户输入的值
    当用户输入RESTART和EXIT时分别会抛出RestartProgramException和
    ExitProgramException
    """
    pl(prompt+"\n抱一抱>>> ",end='')
    answer=input()
    print('')
    if answer=='RESTART':
        raise RestartProgramException()
    elif answer=='EXIT':
        raise ExitProgramException()
    elif answer=='DEBUG':
        pl('DEBUG开关已切换！')
        global showdebug
        showdebug=not showdebug
        return il(prompt)
    return answer

def fakeloading():
    """显示假进度条"""
    i=0.0
    while True:
        i = i + random.random()*5
        if(i>100):
            i=100
        j = -int(-i / 5)
        k = int(i)
        print('[',end='')
        for c in range(20):
            if(c>=j):
                print(load0[c],end='')
            else:
                print(load1[c],end='')
        print('] {0}%'.format(k),end='',flush=True)
        if(i>=100):
            print('\n',end='')
            return
        time.sleep(0.03)
        print('\r',end='')

def findname(prompt):
    """在data中查找人名或缩写

    返回值：(姓名, 缩写, 性别（1男2女）)"""
    global data
    if prompt in data['people']['boys']:
        return (data['people']['boys'][prompt],prompt,1)
    if prompt in data['people']['girls']:
        return (data['people']['girls'][prompt],prompt,2)
    for abbr in data['people']['boys'].keys():
        if prompt == data['people']['boys'][abbr]:
            return (prompt,abbr,1)
    for abbr in data['people']['girls'].keys():
        if prompt == data['people']['girls'][abbr]:
            return (prompt,abbr,2)
    if prompt in data['nameees']:
        return data['nameees'][prompt]

def askname(prompt, self = None):
    '''输入一个名字

    参数：
    prompt(0): 提示文字
    self(1): 自“抱”自弃
    返回值：(姓名, 缩写, 性别（1男2女）)'''
    while True:
        find=findname(il(prompt))
        if find == None:
            pl('花名册上没有找到这个名字...请重试')
            pl('请输入汉字或首字母缩写(小写)！')
        elif isinstance(find,str):
            pl(find)
        elif prompt == self:
            pl('自“抱”自弃')
        else:
            return find

def debug(msg):
    """如果DEBUG开启则输出调试信息

    参数：
    msg(0): 要输出的信息"""
    if(showdebug):
         pl('''
##########DEBUG##########
{0}
#########################
'''.format(msg),delay=0.004)

def bye():
    """按回车退出"""
    il('\n按回车退出...')
    exit()

class RestartProgramException(Exception):
    """用于在中途重新开始程序"""

class ExitProgramException(Exception):
    """用于在中途结束程序"""

def main():
    global t
    if t:
        t.getscreen().bye()
    """主程序"""
    # 输入人名
    find1=askname('你的名字：')
    find2=askname('你想拥抱谁？',find1)
    # 输入要说的话
    pl('你想对[{0}]说什么？'.format(find2[0]))
    message=il('请输入中文！')
    # （调试信息）
    log=''
    if(find1[2]==1):
        g1='男'
    else:
        g1='女'
    if(find2[2]==1):
        g2='男'
    else:
        g2='女'
    log=log+"""Hugger信息
姓名：{0}
缩写：{1}
性别：{2}
Huggee信息
姓名：{3}
缩写：{4}
性别：{5}
要说的话：
{6}

""".format(find1[0],find1[1],g1,find2[0],find2[1],g2,message)

    # 遍历触发条件
    found=False
    debugPersonMatchedBy={(True,False,False):"姓名匹配",(False,True,False):"缩写匹配",(False,False,True):"性别匹配",(False,False,False):"不匹配",None:"任意"} # 调试用
    for condition in data['conditions']: # 开始遍历
        # Hugger
        if 'hugger' in condition:
            hugger=condition['hugger']
            huggerMatch=(hugger==find1[0],hugger==find1[1],hugger==find1[2])
        else:
            hugger=None
            huggerMatch=None
        # Huggee
        if 'huggee' in condition:
            huggee=condition['huggee']
            huggeeMatch=(huggee==find2[0],huggee==find2[1],huggee==find2[2])
        else:
            huggee=None
            huggeeMatch=None
        # Msgre
        if 'msgre' in condition:
            msgre=condition['msgre']
            msgreMatch=re.search(msgre,message) or False
            if msgreMatch:
                msgreDebug="匹配到“{0}”".format(msgreMatch.group())
            else:
                msgreDebug="不匹配"
        else:
            msgre=None
            msgreMatch=None
            msgreDebug="任意"
        # （调试信息）
        log=log+"""遍历触发条件 #{0}
Hugger:{1}（{4}）
Huggee:{2}（{5}）
Msgre:{3}（{6}）

""".format(data['conditions'].index(condition),hugger,huggee,msgre,debugPersonMatchedBy[huggerMatch],debugPersonMatchedBy[huggeeMatch],msgreDebug)
        # 判断是否匹配
        if huggerMatch!=(False,False,False) and huggeeMatch!=(False,False,False) and msgreMatch!=False:
            # 匹配成功
            log=log+'匹配成功' # （调试信息）
            debug(log)
            for line in condition['message']:
                pl(line) # 逐行逐字输出结果
            found=True
            break
    if not found:
        # 未匹配任何条件
        condition=None
        log=log+'未匹配任何条件' # （调试信息）
        debug(log)
        pl('你抱住了{0}'.format(find2[0]))
    runTurtle(find1,find2,message,condition)

def runTurtle(find1,find2,message,condition):
    global t
    """运行Turtle"""
    t=turtle.Turtle()
    t.getscreen().title('抱一抱')
    t.color('red')
    # 红心
    t.left(90)
    t.begin_fill()
    t.circle(10,195)
    t.goto(-16,-11)
    t.goto(-12,-18)
    t.goto(-8,-23)
    t.goto(-4,-27)
    t.goto(0,-30)
    t.up()
    t.goto(0,0)
    t.down()
    t.right(195)
    t.circle(-10,195)
    t.goto(16,-11)
    t.goto(12,-18)
    t.goto(8,-23)
    t.goto(4,-27)
    t.goto(0,-30)
    t.end_fill()
    t.up()
    # Hugger名字
    t.goto(-50,-14)
    t.color(COLORS[find1[2]])
    t.write(find1[0],font=20,align='center')
    #小朋友你是否有很多问号？
    if not (condition and 'confirmed' in condition and condition['confirmed']):
        t.goto(0,20)
        t.color('black')
        t.write('?',font=30,align='center')
    # Huggee名字
    t.goto(50,-14)
    t.color(COLORS[find2[2]])
    t.write(find2[1],font=20,align='center')
    # 说的话
    t.goto(-10,-40)
    t.color(COLORS[find1[2]])
    t.write(message,font=20,align='right')
    # 自定义指令
    if condition and 'eval' in condition:
        exec(condition['eval'])

#########################

pl('读取数据文件...')
try:
    with open("hug.json", 'rb') as fp: # 打开数据文件
        content=fp.read() # 读取文件内容
        fakeloading() # 显示假进度条
        data=json.loads(content) # 解析JSON
except FileNotFoundError:
    # 如果找不到文件
    pl('找不到hug.json，请确认已解压缩且在同一文件夹')
    bye()
except json.decoder.JSONDecodeError as error:
    # 如果解析JSON失败
    pl('hug.json有语法错误，无法读取')
    pl('错误详情：{0}'.format(error))
    bye()
pl('加载完成！')

# 输出credits
pl(r'''CREDITS:
Idea by 孟庆睿
turtle编程 by 孟庆睿
控制台美化 by 罗奕恒
可扩展 by 罗奕恒
-----数据文件元数据------
{0}
FOR: {1}
-------------------------
 |  /---          |  /---
---  _ |         ---  _ |
 |  |_|| _______, |  |_||
--- | _|         --- | _|
 |  |             |  |   
\|   \_/         \|   \_/
{2}
-------------------------
当“抱一抱>>>”提示出现时，
输入RESTART即可重新开始，
输入EXIT即可退出！


'''.format(data['credits'],data['for'],VERSION),delay=0.01)

# 运行主程序
while True:
    try:
        main()
        # 中途如需重新开始或退出，main()会抛出异常
        bye()
        # 结束后如需重新开始，bye()会抛出异常
    except RestartProgramException:
        # 重新开始
        pl('\n---------RESTART---------\n',delay=0.05)
        continue
    except ExitProgramException:
        # 直接关闭
        exit()
    # 如果没有抛出以上两种异常，直接跳出循环
    break

'''
if y==1:
    c.goto(25,-30)
    if gee==1:
        c.color(0.3,0.6,1)
    else:
        c.color(0.9,0.1,0.6)
    c.write('NO')
if y==2:
    c.goto(25,-30)
    c.color(0,0.6,0)
    c.write('NO')
if y==3:
    for aa in range(30):
        c.goto(random.randint(-300,300),random.randint(-300,300))
        c.color(random.randint(0,30)*0.01,random.randint(60,100)*0.01,random.randint(0,30)*0.01)
        c.write('太过分了',font=(30))
'''