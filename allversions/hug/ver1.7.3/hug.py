...
def per(text1,text2):
    while True:
        if mode=="2":
            ...
            a=turtle.textinput("抱一抱_ver1.7.3",text1)
...
def p(text,end="\n"):
    ...
    time.sleep(t1)
turtle.title("抱一抱_ver1.7.3")
mode=turtle.textinput("抱一抱_ver1.7.3","按Enter继续")
t=turtle.Turtle()
if mode in ["1","2"]:
    t1=0.2
    if mode=="1":
...
if mode=="2":
    ...
    while True:
        ee=turtle.textinput("抱一抱_ver1.7.3","是否开启彩蛋模式 (1)是 (2)否 输入一个数字进行选择")
...
while True:
    ...
    if mode=="2":
        ...
        d=turtle.textinput("抱一抱_ver1.7.3","你想对"+b+"说什么?")
    ...
    if a==b:
        ...
        if genderb==2 and ee=="1" and (d[0:4]=="我喜欢上" or d[0:4]=="我愿意上" or d[0:3]=="我想上" or d[0:3]=="我爱上"):
            p("抱一抱_ver1.7.3:“我还是白的吧”")#easteregg1
            y=11
        elif b=="孟庆睿":        
            if a=="武昕冉":
                p("孟庆睿紧紧地抱住了你")
    ...
    if y==11:#yellow
        if yellow<=0.99:
            yellow+=0.01
        turtle.title("抱一抱_ver1.7.3:Yellow")
        t.goto(0,150)
        t.color(1,0.82,0.1)
    ...
    if mode=="2":
        ...
        while True:
            x=turtle.textinput("抱一抱_ver1.7.3","(1)重新开始 (2)退出 (3)关闭彩蛋模式并重新开始 输入一个数字进行选择")
            if x in rd:
                x=choice(["1","3"])
            if x in ["1","2","3"]:
                break
        ...
        while True:
            x=turtle.textinput("抱一抱_ver1.7.3","(1)重新开始 (2)退出 (3)开启彩蛋模式并重新开始 输入一个数字进行选择")
            if x in rd:
                x=choice(["1","3"])
            if x in ["1","2","3"]:
                break
