...
def squBezierTo(tu, p1, p2, splits = 16, rot = True):
    ...
    if rot:
        tu.seth(rad / math.pi * 180)
def cubBezierTo(tu, p1, p2, p3, splits = 16, rot = True):
    p0 = (tu.xcor(), tu.ycor())
    for i in range(splits + 1):
        t = i / splits
        ...
        lineTo(tu, x, y, rot)
    rad = math.atan2(p3[1]-p2[1], p3[0]-p2[0])
    if rot:
        tu.seth(rad / math.pi * 180)
def uiDrawBegin():
    global uiDrawing
...
def keyboardChinese(callback):
    ...
    displayInputStr()
    uiDrawBegin()
    addButton(-240, -64, 32, 32, "上", lambda: keyboardPress("上"), COLORS['tui_normal'])
    addButton(-192, -64, 32, 32, "不", lambda: keyboardPress("不"), COLORS['tui_normal'])
    addButton(-144, -64, 32, 32, "也", lambda: keyboardPress("也"), COLORS['tui_normal'])
    addButton(-96, -64, 32, 32, "你", lambda: keyboardPress("你"), COLORS['tui_normal'])
    addButton(-48, -64, 32, 32, "喜", lambda: keyboardPress("喜"), COLORS['tui_normal'])
    addButton(-240, -112, 32, 32, "对", lambda: keyboardPress("对"), COLORS['tui_normal'])
    addButton(-192, -112, 32, 32, "就", lambda: keyboardPress("就"), COLORS['tui_normal'])
    addButton(-144, -112, 32, 32, "当", lambda: keyboardPress("当"), COLORS['tui_normal'])
    addButton(-96, -112, 32, 32, "成", lambda: keyboardPress("成"), COLORS['tui_normal'])
    addButton(-48, -112, 32, 32, "我", lambda: keyboardPress("我"), COLORS['tui_normal'])
    addButton(-240, -160, 32, 32, "把", lambda: keyboardPress("把"), COLORS['tui_normal'])
    addButton(-192, -160, 32, 32, "是", lambda: keyboardPress("是"), COLORS['tui_normal'])
    addButton(-144, -160, 32, 32, "欢", lambda: keyboardPress("欢"), COLORS['tui_normal'])
    addButton(-96, -160, 32, 32, "爱", lambda: keyboardPress("爱"), COLORS['tui_normal'])
    addButton(-48, -160, 32, 32, "起", lambda: keyboardPress("起"), COLORS['tui_normal'])
    addButton(160, -160, 64, 32, "←Bksp", lambda: keyboardPress("\b"), COLORS['tui_special'])
    addButton(-64, -240, 128, 64, "确定", callback, COLORS['tui_done'])
    addButton(-240, -240, 64, 64, "字母", lambda: keyboard(callback, True), COLORS['tui_special'])
    addButton(96, -240, 128, 64, "重新开始", inputHuggerScreen, COLORS['tui_quit'])
    uiDrawEnd()
    # 上不也你喜对就当成我把是欢爱起 完全已经了
...
def titleScreen():
    ...
    # 开始按钮
    uiDrawBegin()
    addButton(-64, -128, 128, 64, '开始', inputHuggerScreen, COLORS['tui_start'])
    uiDrawEnd()
